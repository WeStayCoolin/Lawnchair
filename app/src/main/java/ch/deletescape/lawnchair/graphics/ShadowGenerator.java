...
public class ShadowGenerator {
    ...
    public synchronized Bitmap recreateIcon(Bitmap icon, int width) {
        // Really ugly hack to still allow calls with custom width
        int tmp = mIconSize;
        mIconSize = width;
        Bitmap bm = recreateIcon(icon, true, mDefaultBlurMaskFilter, AMBIENT_SHADOW_ALPHA,
                KEY_SHADOW_ALPHA);
        mIconSize = tmp;
        return bm;
    }
    ...
    public synchronized Bitmap createShadow(Bitmap icon, int width) {
        int[] offset = new int[2];
